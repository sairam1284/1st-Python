...
# conn.cursor will return a cursor object, you can use this cursor to perform queries. Only pulling India data to save time
cursor = conn.cursor()
...
data_raw['new_avg_temp'] = data_raw['avg_temp'].apply(lambda x: float(x)*1.6 + 32)
data_raw['new_error_temp'] = data_raw['error_temp'].apply(lambda x: float(x)*1.6)
